<?php
include '../FUNCTION/mainFunc.inc.php';

$user_id = $_SESSION['id'];
$connect = OpenCon();

$query = "SELECT * FROM savinggoals WHERE user_id = ? ORDER BY startDate DESC";
$stmt = mysqli_prepare($connect, $query);
mysqli_stmt_bind_param($stmt, "i", $user_id);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="icon" href="../IMG/favicon.png">
    <title>Cash Compass</title>
    <link rel="stylesheet" href="../CSS/set.css">
    <link rel="stylesheet" href="../CSS/sideNav.css">
    <link rel="stylesheet" href="../CSS/shareExpense.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
</head>

<body>
    <?php include '../FUNCTION/sideNav.inc.php'; ?>
    <section id="main">
        <span id="main-title">
            <a href="main.php" id="title-text"><i class="fa-solid fa-money-check"></i> &nbsp;Cash Compass</a>
            <a>SAVING GOALS</a>
            <form action="../FUNCTION/logoutPost.php" method="post" id="logout-form">
                <button id="logout-btn" type="submit">
                    <i class="fa-solid fa-right-from-bracket"></i> Logout
                </button>
            </form>
        </span>
        <hr />
        <section id="share-expense">
            <div id="add-share-expense-container">
                <button onclick="window.location.href='addSavingGoals.php'">ADD SAVING GOAL</button>
            </div>
            <section id="shared-expense-container">
                <?php
                if (mysqli_num_rows($result) === 0) {
                    echo '<div class="shared-info"><p>No saving goals found.</p></div>';
                } else {
                    while ($row = mysqli_fetch_assoc($result)) {
                        $saving_id = $row['saving_id'];
                        $name = htmlspecialchars($row['savingName']);
                        $target = $row['targetAmount'];
                        $current = $row['currentAmount'];
                        $startDate = $row['startDate'];
                        $endDate = $row['endDate'];
                        $status = 'Ongoing';

                        // Calculate status
                        $currentDate = new DateTime();
                        $endDateObj = new DateTime($endDate);
                        $epsilon = 0.01;
                        $remaining = $target - $current;
                        $extra = $current - $target;

                        if ($extra > 0) {
                            $status = 'Completed';
                        } elseif ($currentDate > $endDateObj) {
                            $status = 'Expired';
                        } else {
                            $status = 'Ongoing';
                        }

                        $percent_current = $target > 0 ? min(100, ($current / $target) * 100) : 0;
                        $percent_remaining = 100 - $percent_current;

                        echo '<div class="shared-info">';
                        echo '  <div class="shared-expense-header">';
                        echo '    <h3>' . $name . '</h3>';
                        echo '    <p style="font-size:1em; text-align:right; padding:0; margin:0;"><span style="color:' .
                            ($status === 'Completed' ? '#2e7d32' : ($status === 'Expired' ? '#b71c1c' : '#1e3a5c')) .
                            '">' . $status . '</span></p>';
                        echo '  </div>';
                        echo '  <div class="shared-expense-content">';
                        // Show total progress bar for the saving goal
                        echo '<div class="progress-bar-container">';
                        if ($percent_current < 30) {
                            echo '  <div class="progress-bar-filled" style="width: ' . $percent_current . '%;">';
                            echo '  </div>';
                            echo '  <div class="progress-bar-remaining" style="width: ' . $percent_remaining . '%; display: flex; align-items: center; justify-content: flex-start; padding-left: 0.7em; font-size: 0.7em; color: #5c4442;">';
                            echo '    <span class="progress-bar-text">RM ' . number_format($current,2) . ' saved</span>';
                            echo '  </div>';
                        } else {
                            echo '  <div class="progress-bar-filled" style="width: ' . $percent_current . '%; text-align: right; padding-right: 0.7em; font-size: 0.7em; color: #5c4442; align-items: center; display: flex; justify-content: flex-end;">';
                            echo '    <span class="progress-bar-text">RM ' . number_format($current,2) . ' saved</span>';
                            echo '  </div>';
                            echo '  <div class="progress-bar-remaining" style="width: ' . $percent_remaining . '%"></div>';
                        }
                        echo '  <span class="shared-expense-amount">RM ' . number_format($target,2) . ' target</span>';
                        echo '</div>';
                        echo '    <button class="view-details-btn" onclick="window.location.href=\'viewSavingGoalDetails.php?id=' . $saving_id . '\'">VIEW DETAILS</button>';
                        echo '  </div>';
                        echo '</div>';
                    }
                }
                ?>
            </section>
        </section>
    </section>
</body>

</html>